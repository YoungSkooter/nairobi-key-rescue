    ...
    <div className="min-h-screen bg-background text-foreground">
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container text-center px-4 py-20">
          ...
          <h1 className="font-heading text-4xl md:text-6xl font-bold uppercase tracking-tight mb-4">Emergency Car <span className="text-primary">Locksmith</span> Nairobi</h1>
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-8">Locked out of your car? We'll be there fast. 24/7 emergency car lockout service across Nairobi.</p>
          ...
          <p className="text-sm text-muted-foreground"><MapPin className="w-4 h-4 inline mr-1" /> Ngong Rd, Adam's Arcade • We Come to You Anywhere</p>
        </div>
      </section>

      <section className="py-16 md:py-24 bg-card">
        <div className="container px-4">
          <h2 className="font-heading text-3xl md:text-4xl font-bold uppercase text-center mb-12">Fast <span className="text-primary">Emergency</span> Response</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div><img src={duplication} alt="Emergency lockout assistance" className="rounded-lg w-full h-64 object-cover" loading="lazy" /></div>
            <div className="flex flex-col justify-center">
              <ul className="space-y-4">
                {reasons.map((r) => (<li key={r} className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-primary mt-0.5 shrink-0" /><span className="text-muted-foreground">{r}</span></li>))}
              </ul>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-secondary">
        <div className="container px-4 text-center">
          <h2 className="font-heading text-3xl md:text-5xl font-bold uppercase mb-4">Locked Out <span className="text-primary">Right Now?</span></h2>
          <p className="text-muted-foreground text-lg mb-8 max-w-xl mx-auto">Don't panic. Call us and we'll be there in minutes.</p>
